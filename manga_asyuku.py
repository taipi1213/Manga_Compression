...
class CaesiumCLTGUI(TkinterDnD.Tk):
    def __init__(self):
        ...
        # --- 複数フォルダ構造のファイル処理オプション ---
        self.move_multi_folder_files = tk.BooleanVar(value=False)  # 複数フォルダ構造のファイルを移動するオプション
        self.multi_folder_destination = tk.StringVar()  # 移動先フォルダ
    ...
    def create_widgets(self):
        ...
        error_label = tk.Label(error_frame, text="エラー・スキップログ", fg="red")
    ...
    def save_config(self):
        ...
        config = {
            ...
            # 複数フォルダ構造ファイル処理設定
            "move_multi_folder_files": self.move_multi_folder_files.get(),
            "multi_folder_destination": self.multi_folder_destination.get()
        }
    ...
    def load_config(self):
        ...
        try:
            ...
            # 複数フォルダ構造ファイル処理設定の読み込み
            if "move_multi_folder_files" in config:
                self.move_multi_folder_files.set(config.get("move_multi_folder_files", False))
            if "multi_folder_destination" in config:
                self.multi_folder_destination.set(config.get("multi_folder_destination", ""))
    ...
    def create_output_tab(self, parent):
        ...
        # 複数フォルダ構造ファイル処理フレーム（新規追加）
        self.multi_folder_frame = ttk.LabelFrame(parent, text="複数フォルダ構造のファイル処理")
        self.multi_folder_frame.pack(fill=tk.X, padx=5, pady=5)
        
        # チェックボックス
        tk.Checkbutton(
            self.multi_folder_frame, 
            text="複数フォルダ構造のファイルを指定場所に移動する", 
            variable=self.move_multi_folder_files
        ).pack(anchor="w", padx=5, pady=2)
        
        # 移動先フォルダ選択
        dest_frame = tk.Frame(self.multi_folder_frame)
        dest_frame.pack(fill=tk.X, padx=5, pady=5)
        tk.Label(dest_frame, text="移動先フォルダ:").pack(side=tk.LEFT)
        tk.Entry(dest_frame, textvariable=self.multi_folder_destination, width=30).pack(side=tk.LEFT, padx=5)
        tk.Button(
            dest_frame, 
            text="参照", 
            command=self.select_multi_folder_destination
        ).pack(side=tk.LEFT)

    def select_multi_folder_destination(self):
        """複数フォルダ構造のファイルの移動先フォルダを選択"""
        if self.processing:
            return
            
        folder = filedialog.askdirectory(title="複数フォルダ構造ファイルの移動先を選択")
        if folder:
            self.multi_folder_destination.set(folder)
            self.log(f"複数フォルダ構造ファイルの移動先: {folder}")
        ...
        for p in paths:
            ...
            if os.path.isdir(p):
                for root, _, files in os.walk(p):
                    for f in files:
                        ext = os.path.splitext(f)[1].lower()
                        if ext in [".jpg", ".jpeg", ".png", ".webp", ".tiff", ".gif", ".bmp", ".zip", ".rar"]:
    ...
    def extract_archive(self, inpath, ext, out_dir):
        ...
        try:
            if ext == ".zip":
                ...
                if is_multi_folder:
                    self.log(LANG["multiple_folders"].format(inpath))
                    self.log_skip(os.path.basename(inpath), "複数フォルダ構造", inpath)
                    self.size_summary[inpath] = (os.path.getsize(inpath), 0, True, "複数フォルダ構造")
                    
                    # 複数フォルダ構造のファイルを移動するオプションが有効な場合
                    if self.move_multi_folder_files.get() and self.multi_folder_destination.get():
                        dest_folder = self.multi_folder_destination.get()
                        if os.path.exists(dest_folder):
                            try:
                                # 移動先のファイルパスを作成
                                dest_path = os.path.join(dest_folder, os.path.basename(inpath))
                                
                                # 既に同名ファイルがある場合の処理
                                if os.path.exists(dest_path):
                                    base_name = os.path.splitext(os.path.basename(inpath))[0]
                                    ext_name = os.path.splitext(inpath)[1]
                                    counter = 1
                                ...
                                def try_delete_file():
                                    ...
                                    try:
                                        # 方法1: 通常のコピー＆削除
                                        self.log(f"複数フォルダ構造のファイルをコピー中: {inpath} → {dest_path}")
                                        shutil.copy2(inpath, dest_path)  # 内容とメタデータをコピー
                                        
                                        # さらに明示的にメタデータをコピー
                                        if os.path.exists(dest_path):
                                            try:
                                                shutil.copystat(inpath, dest_path)  # メタデータを明示的にコピー
                                                # アクセス時刻と変更時刻を保持
                                                stat = os.stat(inpath)
                                                os.utime(dest_path, (stat.st_atime, stat.st_mtime))
                                                self.log(f"メタデータをコピーしました")
                                            except Exception as stat_err:
                                                self.log(f"メタデータのコピーに失敗: {str(stat_err)}")
                                        ...
                                        self.log(f"方法1での削除失敗: {str(e1)}")
                                        
                                        try:
                                            # 方法2: send2trashを使用
                                            if not os.path.exists(dest_path):
                                                shutil.copy2(inpath, dest_path)
                                                # メタデータを明示的にコピー
                                                try:
                                                    shutil.copystat(inpath, dest_path)
                                                    stat = os.stat(inpath)
                                                    os.utime(dest_path, (stat.st_atime, stat.st_mtime))
                                                    self.log(f"メタデータをコピーしました")
                                                except:
                                            ...
                                            self.log(f"方法2での削除失敗: {str(e2)}")
                                            
                                            try:
                                                # 方法3: WindowsコマンドでDEL使用
                                                if not os.path.exists(dest_path):
                                                    shutil.copy2(inpath, dest_path)
                                                    # メタデータを明示的にコピー
                                                    try:
                                                ...
                                                if os.path.exists(dest_path):
                                                    time.sleep(1.0)
                                                    import subprocess
                                                    subprocess.run(['cmd', '/c', f'del /f "{inpath}"'], 
                                                                  shell=True, check=False)
                                                    return not os.path.exists(inpath)
                                            except Exception as e3:
                                                self.log(f"方法3での削除失敗: {str(e3)}")
                                    return False
                                
                                # ファイル削除を試みる
                                delete_success = try_delete_file()
                                
                                if delete_success:
                                    self.log(f"元ファイル削除成功: {inpath}")
                                else:
                                    self.log_error(f"元ファイル削除できませんでした。後で手動削除してください: {inpath}")
                                
                                # 処理対象からは必ず削除
                                if inpath in self.input_files:
                                    self.input_files.remove(inpath)
                                    self.log(f"処理対象から削除: {inpath}")
                                    
                                # UIを更新して処理対象ファイルを反映
                                self.run_on_ui_thread(self.update_file_count)
                                    
                            except Exception as e:
                                self.log_error(f"ファイル移動/コピーエラー: {str(e)}", inpath)
                                import traceback
                                self.log_error(f"詳細エラー: {traceback.format_exc()}", inpath)
            ...
            # RARファイルの処理
                ...
                if is_multi_folder:
                    ...
                    self.log(LANG["multiple_folders"].format(inpath) + " (サブフォルダに複数構造)")
                    self.log_skip(os.path.basename(inpath), "サブフォルダに複数構造", inpath)
                    self.size_summary[inpath] = (os.path.getsize(inpath), 0, True, "サブフォルダに複数構造")
    ...
    def compress_images_flat(self, src_folder, dst_folder, archive_file=None, pause_callback=None):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for src_path in image_files:
                ...
                future = executor.submit(compress_image_worker, src_path, dst_folder, **params)
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    if result[2]:  # エラーがある場合
                        self.log_error(f"処理エラー: {result[2]}", archive_file)
                except Exception as e:
                    self.log_error(f"エラー発生: {e}", archive_file)
    ...
    def log_error(self, message, archive_file=None):
        ...
        # エラーを記録用リストに保存
        if not hasattr(self, '_recorded_errors'):
            self._recorded_errors = []
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        self._recorded_errors.append(f"[{timestamp}] {full_message}")
    ...
    def log_skip(self, filename, reason, archive_file=None):
        ...
        if not hasattr(self, '_recorded_errors'):
            self._recorded_errors = []
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        self._recorded_errors.append(f"[{timestamp}] {message}")
